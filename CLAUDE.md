...
**Current state (Phase 4 complete — backend + payments done, mobile + audit still ahead):** local-first encrypted vault, optional cloud sync, accounts, multi-device, multi-currency balance sheet, full release engine (Shamir SSS + X25519 + 14-day hold + multi-channel alerts + nominee combine), Razorpay subscriptions with GST-compliant tax invoices, free-tier gating (10 vault items, no release plan on free). All of this activates when `VITE_SUPABASE_URL` + `VITE_SUPABASE_ANON_KEY` are set at deploy time + the SQL migrations are applied. Without those env vars, the app remains local-only — Phase 0 behaviour. See [ROADMAP.md](ROADMAP.md), [SETUP.md](SETUP.md), and [docs/death-simulation-runbook.md](docs/death-simulation-runbook.md).
