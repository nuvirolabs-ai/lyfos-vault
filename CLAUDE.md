...
**Current state (Phase 3 complete — backend complete, mobile + audit + payments still ahead):** local-first encrypted vault with optional cloud sync, accounts, multi-device, multi-currency balance sheet, full release engine (Shamir SSS + X25519 + 14-day hold + multi-channel alerts + nominee combine). All of this activates when `VITE_SUPABASE_URL` + `VITE_SUPABASE_ANON_KEY` are set at deploy time + the SQL migrations are applied. Without those env vars, the app remains local-only — Phase 0 behaviour. See [ROADMAP.md](ROADMAP.md) for the full plan, [docs/os-one-prd.md](docs/os-one-prd.md) for the long-form product vision, [SETUP.md](SETUP.md) for operator setup, and [docs/death-simulation-runbook.md](docs/death-simulation-runbook.md) for the end-to-end release acceptance test.
