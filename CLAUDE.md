...
**Current state (Phase 7 code-side complete — marketing site, onboarding email sequence, press kit, founding-members cohort plan, launch-day checklist all in repo; external audit + insurance procurement + the actual Tuesday launch still to commission):** local-first encrypted vault, optional cloud sync, accounts, multi-device, multi-currency balance sheet, full release engine (Shamir SSS + X25519 + 14-day hold + multi-channel alerts including mobile push + nominee combine), Razorpay subscriptions with GST-compliant tax invoices, free-tier gating (10 vault items, no release plan on free), iOS + Android apps via Expo SDK 54 with Face ID / Touch ID unlock and Universal Links into the same release routes. All of this activates when `VITE_SUPABASE_URL` + `VITE_SUPABASE_ANON_KEY` (web) / `EXPO_PUBLIC_SUPABASE_URL` + `EXPO_PUBLIC_SUPABASE_ANON_KEY` (mobile) are set at build time + the SQL migrations are applied. See [ROADMAP.md](ROADMAP.md), [SETUP.md](SETUP.md), [apps/mobile/README.md](apps/mobile/README.md), and [docs/death-simulation-runbook.md](docs/death-simulation-runbook.md). Phase 6 trust/compliance/runbook pack lives under [docs/security/](docs/security/), [docs/compliance/](docs/compliance/), and [docs/runbooks/](docs/runbooks/); public counterparts at `/security/`, `/legal/sub-processors.html`, `/status/`, and `/.well-known/security.txt`. Phase 7 marketing site is a standalone static site under [apps/marketing/](apps/marketing/) (deploys to `lyfos.com`); launch content under [docs/marketing/](docs/marketing/) covers the 7-email onboarding sequence (Edge Function at `supabase/functions/onboarding-emails/` + migration `0014`), press kit, founding-members cohort plan, journalist outreach list, and launch-day checklist.
